    ...
    def parse(self):
        ...
        for number, line in iterate_lines(self.path):
            ...
            if declare_track:
                declare_track = False
                if last_feature:
                    self.handler.newFeature(last_feature)
                    last_feature = None
            ...
            if params['mode'] == 'fixedStep':
                ...
                feature = [params['chrom'], base, base + params['span'], line]
                params['count'] += 1
            # Variable #
            if params['mode'] == 'variableStep':
                ...
                feature = [params['chrom'], line[0], line[0] + params['span'], line[1]]
            # Ignore null scores #
            ...
            if not last_feature: last_feature = feature
            else:
                if last_feature[0] == feature[0]:
                    if last_feature[2] > feature[1]:
                        self.handler.error(self.path, -1, "The track%s has a start or span larger than its end or step.")
                    if last_feature[2] == feature[1] and last_feature[3] == feature[3]:
                        last_feature[2] = feature[2]
                        continue
                self.handler.newFeature(last_feature)
                last_feature = feature
        if last_feature: self.handler.newFeature(last_feature)
